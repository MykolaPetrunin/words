...
import { stopLearningBook } from '@/lib/repositories/bookRepository';
...
const mockStopLearningBook = stopLearningBook as jest.MockedFunction<typeof stopLearningBook>;

describe('POST /api/books/[id]/learning/stop', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });

    it('should stop learning a book successfully', async () => {
        ...
        mockStopLearningBook.mockResolvedValue(mockBook);
        ...
        expect(mockStopLearningBook).toHaveBeenCalledWith(mockUser.id, 'book-1');
    });
    ...
    it('should return 500 when book is not found', async () => {
        ...
        mockGetServerSession.mockResolvedValue(mockSession);
        mockGetUserByFirebaseId.mockResolvedValue(mockUser);
        mockStopLearningBook.mockRejectedValue(new Error('Book not found'));
    ...
    it('should handle server errors', async () => {
        ...
        mockGetServerSession.mockResolvedValue(mockSession);
        mockGetUserByFirebaseId.mockResolvedValue(mockUser);
        mockStopLearningBook.mockRejectedValue(new Error('Database error'));
