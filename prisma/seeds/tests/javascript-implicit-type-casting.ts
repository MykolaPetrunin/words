        ...
        },
        {
            ...
            answers: [
                ...
                },
                {
                    ...
                    theoryUK: '✅ Коректна для виразів, явно записаних у дужках: операнди трактуються як об’єкти (не як блок коду).',
                    theoryEN: '✅ Correct for expressions explicitly wrapped in parentheses: operands are treated as objects (not as a code block).',
                    isCorrect: true
                },
                {
                    textUK: '`[] + []` → 0, `[] + {}` → NaN, `{} + []` → NaN',
                    textEN: '`[] + []` → 0, `[] + {}` → NaN, `{} + []` → NaN',
                    theoryUK: '❌ Неправильно. Бінарний `+` між об’єктами спершу приводить їх до рядків (через `toString`/`valueOf`/`ToPrimitive`), а не до чисел.',
                    theoryEN: '❌ Incorrect. Binary `+` with objects coerces them to strings first (via `toString`/`valueOf`/`ToPrimitive`), not numbers.',
                    isCorrect: false
                }
            ],
            level: 'middle'
